...
import { deleteEvent, toggleEventDestaque } from "@/lib/events/actions"
import {
  EVENT_CATEGORIAS,
  isUpcoming,
  type BangEvent,
...
interface EventosManagerProps {
  initialEvents: BangEvent[]
}

/**
 * Client component that manages the interactive eventos dashboard. Receives
 * the initial list from the Server Component and refreshes via router.refresh()
 * after each mutation (delete, destaque toggle) or after the form modal saves.
 */
export function EventosManager({ initialEvents }: EventosManagerProps) {
  ...
  const handleModalSaved = () => {
    // Server action already called revalidatePath — just refresh the RSC tree.
  ...
  const handleConfirmDelete = async () => {
    ...
    const result = await deleteEvent(confirmingDelete.id)
    if (!result.ok) {
      // Minimal inline error — a toast system can be added later.
      alert(result.error ?? "Erro ao remover evento.")
      return
    }
    startTransition(() => router.refresh())
  }
  const handleToggleDestaque = async (event: BangEvent) => {
    await toggleEventDestaque(event.id, !event.destaque)
    startTransition(() => router.refresh())
  }

  // Quick-action deep link: ?new=1 opens the create modal.
  ...
  return (
    <>
      ...
      <div className={cn("px-5 md:px-8 py-6 md:py-8 flex flex-col gap-5", isPending && "opacity-70 pointer-events-none")}>
        {events.length === 0 ? (
          <EmptyPanel onCreate={handleCreate} />
        ) : (
          <>
              ...
              <ul className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 items-stretch">
                {filtered.map((event) => (
                  <EventRow
                    ...
                    onToggleDestaque={() => handleToggleDestaque(event)}
                  />
                ))}
              </ul>
            )}
