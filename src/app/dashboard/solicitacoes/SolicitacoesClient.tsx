...
import { deleteWishlistRequest } from "@/lib/wishlist/actions"
import { whatsappUrl } from "@/lib/contacts/useContacts"
import { useMobileMenu } from "../mobile-menu-context"
import type { CityRequest, CityRankRow } from "@/lib/wishlist/config"
...
export function SolicitacoesClient({ requests, ranking }: SolicitacoesClientProps) {
  ...
  const handleConfirmDelete = () => {
    ...
    startTransition(async () => {
      const result = await deleteWishlistRequest(target.id)
      if (!result.ok) {
        ...
        >
          ...
          >
            ...
            {deleteError && (
              <div
                role="alert"
                className="mt-3 rounded-xl px-4 py-3 text-sm bg-[#FEE2E2] border border-[#FCA5A5] text-[#991B1B]"
              >
                {deleteError}
              </div>
            )}
            <div className="flex items-center justify-end gap-2 mt-6">
              <button
                ...
                onClick={() => setConfirmingDelete(null)}
                disabled={isPending}
                className="h-10 px-4 rounded-lg text-[13px] font-semibold text-[#4A2C1A]/75 hover:bg-[#FAFAF8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#E87A1E] disabled:opacity-50"
              >
                Cancelar
              </button>
              <button
                ...
                disabled={isPending}
                className="h-10 px-4 rounded-lg text-[13px] font-semibold bg-[#D32F2F] text-white hover:bg-[#B71C1C] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D32F2F] focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
