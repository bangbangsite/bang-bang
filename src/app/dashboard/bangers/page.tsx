...
export default function DashboardBangersPage() {
  const {
    applications,
    total,
    setStatus,
  ...
  // Pipeline counts (off the full list, not filtered) — drives the status chip strip
  ...
  const filtered = useMemo(() => {
    ...
    return applications
      ...
      .sort((a, b) => {
        // Favorites first, then most recent
  ...
  const handleReset = () => {
    if (window.confirm(`Apagar todos os ${total} candidatos? Essa ação não pode ser desfeita.`)) {
      resetAll()
    }
  }
    ...
    <>
      ...
      <div className="px-5 md:px-8 py-6 md:py-8 flex flex-col gap-5">
        {applications.length === 0 ? (
          ...
          <>
            {/* Pipeline status strip — quick filter pinned to the top */}
            <section
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 align-middle text-[12px] text-[#4A2C1A]/75">
                              {a.cidade}
                              {a.uf && (
                                <span className="text-[#4A2C1A]/45">/{a.uf}</span>
                              )}
                            </td>
                            <td className="px-4 py-3 align-middle">
      ...
      <BangerDetailModal
        ...
        onUpdateNotes={(id, notas) => updateApplication(id, { notas })}
        onDelete={(id) => removeApplication(id)}
      />
    </>
  )
}
  ...
  return (
    <div className="rounded-2xl bg-white border border-dashed border-[#4A2C1A]/15 p-12 text-center flex flex-col items-center gap-3">
      ...
      >
        ...
        Popular com 6 exemplos
      </button>
      <p className="text-[11px] text-[#4A2C1A]/50">
        Dados fake pra testar pipeline e filtros.
      </p>
    </div>
  )
}
